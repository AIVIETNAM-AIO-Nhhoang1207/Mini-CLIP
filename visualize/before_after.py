"""
So sánh Cosine Similarity trước và sau khi train.
Hiển thị 2 heatmap cạnh nhau: Random Weights vs Trained Weights.
"""
import os
...
def main():
    ...
    with torch.no_grad():
        ...
        # Trước khi train (random weights)
        img_feat_before, txt_feat_before, _ = model(
            images, text_tokens["input_ids"], text_tokens["attention_mask"],
        )
        sim_matrix_before = (
            F.normalize(img_feat_before, p=2, dim=-1)
            @ F.normalize(txt_feat_before, p=2, dim=-1).T
        ).cpu().numpy()

        # Sau khi train (loaded weights)
        print("Loading trained weights...")
        model.load_state_dict(
            torch.load("best_mini_clip.pth", map_location=device, weights_only=True)
        )

        img_feat_after, txt_feat_after, _ = model(
            images, text_tokens["input_ids"], text_tokens["attention_mask"],
        )
        sim_matrix_after = (
            F.normalize(img_feat_after, p=2, dim=-1)
            @ F.normalize(txt_feat_after, p=2, dim=-1).T
        ).cpu().numpy()
    ...
    stacked_images = np.vstack(img_list)
    ...
    sns.heatmap(
        sim_matrix_before, annot=True, cmap="coolwarm", fmt=".2f",
        xticklabels=short_captions, yticklabels=False, ax=ax_before, cbar=False,
    )
    ax_before.set_title(
        "BEFORE Training (Random Weights)", fontsize=13,
        color="red", pad=15, fontweight="bold",
    )
    ...
    sns.heatmap(
        sim_matrix_after, annot=True, cmap="coolwarm", fmt=".2f",
        xticklabels=short_captions, yticklabels=False, ax=ax_after,
    )
    ax_after.set_title(
        "AFTER Training (Trained Weights)", fontsize=13,
        color="green", pad=15, fontweight="bold",
    )
